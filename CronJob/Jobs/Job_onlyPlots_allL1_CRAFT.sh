...
last_file=$(find $rate_plot_dir"plots/2024_CRAFT_allL1" -maxdepth 1 -type f -printf '%T@ %p\n' | sort -n | tail -1 | cut -d' ' -f2-)

if [ ${oms_ntuplizer_dir}/"2024_CRAFT_merged.root" -nt "$last_file" ]
then
    command="cd ${rate_plot_dir} && python3 trigger_plots.py --cosmics --rates  --vsRun --vsFill --vsTime  --lumisPerBin 30 --inputFile \"${oms_ntuplizer_dir}/2024_CRAFT_merged.root\" --triggers allL1  --selections \"2024_CRAFT_allL1=fill>9517\""
    echo $command
    eval $command
else
